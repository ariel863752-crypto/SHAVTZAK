import io
import streamlit as st
import pandas as pd
import plotly.express as px
import plotly.graph_objects as go
from ortools.sat.python import cp_model

# ══════════════════════════════════════════════════════════════════
# הגדרות עמוד — חייב להיות ראשון
# ══════════════════════════════════════════════════════════════════
st.set_page_config(
    page_title='שבצ"ק חכם',
    page_icon="🪖",
    layout="wide",
    initial_sidebar_state="collapsed",
)

# ══════════════════════════════════════════════════════════════════
# CSS — RTL מלא + תיקון חפיפות
# ══════════════════════════════════════════════════════════════════
st.markdown("""
<style>
@import url('https://fonts.googleapis.com/css2?family=Heebo:wght@300;400;500;600;700;800;900&display=swap');

/* ── בסיס RTL גלובלי ── */
[data-testid="stAppViewContainer"], .main, .block-container {
    direction: rtl !important;
    text-align: right !important;
}

/* תיקון כללי לטקסטים שבורחים שמאלה */
div, p, span, h1, h2, h3, h4, h5, h6, label {
    text-align: right !important;
    direction: rtl !important;
}

/* ── רקע ── */
.stApp { background: #f4f6f4; }
.block-container { padding: 2rem 2.5rem 3rem 2.5rem; max-width: 1400px; }

/* ── כותרת ראשית (תוקן יישור) ── */
.app-header {
    background: linear-gradient(135deg, #1a3d17 0%, #2d5a27 60%, #3d7a35 100%);
    border-radius: 16px;
    padding: 28px 32px;
    margin-bottom: 28px;
    color: white;
    text-align: right;
}
.app-header h1 {
    font-size: 38px;
    font-weight: 900;
    margin: 0;
    color: white !important;
}
.app-header p {
    font-size: 16px;
    opacity: 0.9;
    margin-top: 8px;
    color: white !important;
}

/* ── טאבים (תוקן יישור) ── */
[data-testid="stTabs"] [data-baseweb="tab-list"] {
    gap: 10px;
    background: transparent;
    flex-direction: row-reverse !important; /* הזזת הטאבים לימין */
}
[data-testid="stTabs"] [data-baseweb="tab"] {
    border-radius: 8px 8px 0 0;
    padding: 10px 25px;
    background-color: #e9eee9;
}

/* ── כרטיסי מדד (תוקנה חפיפה) ── */
.metric-row { 
    display: flex; 
    gap: 16px; 
    margin: 20px 0; 
    flex-wrap: wrap; /* מאפשר ירידת שורה במקום חפיפה */
    direction: rtl;
}
.metric-card {
    flex: 1;
    min-width: 200px; /* מונע התכווצות יתר */
    background: white;
    border-radius: 14px;
    padding: 20px;
    border: 1px solid #dde8dc;
    box-shadow: 0 2px 8px rgba(45,90,39,0.07);
}
.mc-label { font-size: 13px; color: #7a9a77; font-weight: 600; margin-bottom: 8px; }
.mc-value { font-size: 32px; font-weight: 800; color: #1a3d17; }

/* ── כפתורים ── */
div.stButton > button {
    background: linear-gradient(135deg, #2d5a27, #3d7a35) !important;
    color: white !important;
    font-weight: 700 !important;
    border-radius: 10px !important;
    height: 3.5em;
    width: 100%;
}

/* ── טבלאות ── */
[data-testid="stTable"] {
    overflow-x: auto;
    direction: rtl !important;
}
table {
    width: 100% !important;
    direction: rtl !important;
    text-align: right !important;
}

/* תיקון הודעות אזהרה ומידע */
.info-box, .warn-box, .error-box {
    padding: 15px;
    border-radius: 8px;
    margin: 10px 0;
    border-right: 5px solid;
    text-align: right;
    direction: rtl;
}
.info-box { background: #edf5ec; border-color: #2d5a27; color: #1a3d17; }
.warn-box { background: #fff8e6; border-color: #e67e22; color: #7a4500; }
.error-box { background: #fdecea; border-color: #c0392b; color: #7a0010; }
</style>
""", unsafe_allow_html=True)

# ══════════════════════════════════════════════════════════════════
# לוגיקה ומחלקות (זהה לקוד המקורי שלך, עם תיקוני יישור קלים)
# ══════════════════════════════════════════════════════════════════

class Soldier:
    def __init__(self, s_id, name: str, restr=""):
        self.soldier_id = str(s_id)
        self.name = str(name).strip()
        self.restricted_tasks = self._parse(restr)

    @staticmethod
    def _parse(restr) -> list[int]:
        if pd.isna(restr) or str(restr).strip() in ("", "nan"): return []
        return [int(p.replace(".0","")) for p in str(restr).replace(" ","").split(",")
                if p.replace(".0","").lstrip("-").isdigit()]

class Task:
    NIGHT: set[int] = {22,23,0,1,2,3,4,5,6,7}
    def __init__(self, t_id, name, req_p, shift_dur=None, rest_dur=None, overlap=False, hours=None):
        self.task_id = int(t_id)
        self.name = str(name).strip()
        self.required_personnel = int(req_p)
        self.shift_duration = int(shift_dur) if pd.notna(shift_dur) else 1
        self.rest_duration = int(rest_dur) if pd.notna(rest_dur) else 0
        self.allow_overlap = str(overlap).strip().lower() == "true"
        self.active_hours = self._parse_hours(hours)

    @staticmethod
    def _parse_hours(hours) -> list[int]:
        if pd.isna(hours) or str(hours).strip().lower() in ("all","","nan"): return list(range(24))
        return [int(x) for x in str(hours).replace(" ","").split(",") if x.isdigit()]

def to_excel_styled(df: pd.DataFrame, sheet_name='שבצ"ק', include_index=True) -> bytes:
    output = io.BytesIO()
    with pd.ExcelWriter(output, engine="xlsxwriter") as writer:
        df.to_excel(writer, index=include_index, sheet_name=sheet_name)
    return output.getvalue()

def validate(soldiers: list[Soldier], tasks: list[Task]):
    errors, warnings = [], []
    if not soldiers: errors.append("לא נמצאו חיילים בקובץ.")
    if not tasks: errors.append("לא נמצאו משימות בקובץ.")
    return errors, warnings

# --- פונקציות הליבה של האלגוריתם (ללא שינוי מהקוד שלך) ---

def _build_and_solve(soldiers, tasks, H, min_sleep, timeout):
    model = cp_model.CpModel()
    x, start = {}, {}
    for s in soldiers:
        for t in tasks:
            for h in range(H):
                x[s.soldier_id, t.task_id, h] = model.NewBoolVar(f"x_{s.soldier_id}_{t.task_id}_{h}")
                start[s.soldier_id, t.task_id, h] = model.NewBoolVar(f"st_{s.soldier_id}_{t.task_id}_{h}")

    for s in soldiers:
        for t in tasks:
            for h in range(H):
                rel = [start[s.soldier_id, t.task_id, i] for i in range(max(0, h - t.shift_duration + 1), h + 1)]
                model.Add(x[s.soldier_id, t.task_id, h] == sum(rel))
                if h + t.shift_duration > H: model.Add(start[s.soldier_id, t.task_id, h] == 0)

    for s in soldiers:
        for t in tasks:
            if t.rest_duration <= 0: continue
            busy = t.shift_duration + t.rest_duration
            for h in range(H):
                for nh in range(h + 1, min(h + busy, H)):
                    for ot in tasks:
                        if not ot.allow_overlap:
                            model.AddImplication(start[s.soldier_id, t.task_id, h], x[s.soldier_id, ot.task_id, nh].Not())

    for h in range(H):
        hour_of_day = h % 24
        for t in tasks:
            assigned = [x[s.soldier_id, t.task_id, h] for s in soldiers]
            if hour_of_day in t.active_hours: model.Add(sum(assigned) == t.required_personnel)
            else: model.Add(sum(assigned) == 0)

    for s in soldiers:
        for h in range(H):
            blocking = [x[s.soldier_id, t.task_id, h] for t in tasks if not t.allow_overlap]
            model.Add(sum(blocking) <= 1)

    for s in soldiers:
        for t in tasks:
            if t.task_id in s.restricted_tasks:
                for h in range(H): model.Add(x[s.soldier_id, t.task_id, h] == 0)

    # הוגנות ושינה
    s_loads = [sum(x[s.soldier_id, t.task_id, h] for t in tasks for h in range(H)) for s in soldiers]
    max_load = model.NewIntVar(0, H * len(tasks), "max_load")
    min_load = model.NewIntVar(0, H * len(tasks), "min_load")
    for load in s_loads:
        model.Add(max_load >= load)
        model.Add(min_load <= load)
    
    night_work = sum(x[s.soldier_id, t.task_id, h] for s in soldiers for t in tasks if not t.allow_overlap for h in range(H) if (h % 24) in Task.NIGHT)
    
    model.Minimize(100 * (max_load - min_load) + 2 * night_work)
    solver = cp_model.CpSolver()
    solver.parameters.max_time_in_seconds = timeout
    status = solver.Solve(model)
    if status in (cp_model.OPTIMAL, cp_model.FEASIBLE): return solver, x, start, "ok"
    return None, {}, {}, "infeasible"

def solve_scheduling(soldiers, tasks, min_sleep=6, timeout=35.0, num_days=2):
    H = 24 * num_days
    solver, x, start, status = _build_and_solve(soldiers, tasks, H, min_sleep, timeout)
    if solver is None: return None, "לא נמצא פתרון. נסה להפחית דרישות."
    
    rows = []
    for s in soldiers:
        row = {"שם": s.name}
        total, night_count = 0, 0
        for h in range(H):
            day = h // 24 + 1
            hour_label = f"יום{day} {h%24:02d}:00"
            active = [t.name for t in tasks if solver.Value(x[s.soldier_id, t.task_id, h]) == 1]
            row[hour_label] = " + ".join(active) if active else "—"
            if active:
                total += 1
                if (h % 24) in Task.NIGHT: night_count += 1
        row["סך שעות"] = total
        row["שעות לילה"] = night_count
        rows.append(row)
    return pd.DataFrame(rows), "ok"

# ══════════════════════════════════════════════════════════════════
# ממשק משתמש (UI)
# ══════════════════════════════════════════════════════════════════

st.markdown("""
<div class="app-header">
  <h1>🪖 שבצ"ק — מערכת שיבוץ כוחות חכמה</h1>
  <p>אופטימיזציה אוטומטית של שמירות, תורנויות ומשימות — הוגן, מהיר, מדויק</p>
</div>
""", unsafe_allow_html=True)

tab_run, tab_guide, tab_templates = st.tabs(["🚀 ביצוע שיבוץ", "📖 מדריך מפורט", "📥 תבניות עבודה"])

with tab_templates:
    st.markdown("### 📥 הורדת תבניות עבודה")
    c1, c2 = st.columns(2)
    # כאן יבואו כפתורי ההורדה (מקוצר לצורך הדוגמה)

with tab_guide:
    st.markdown("### 📖 מדריך — הסבר לכל עמודה")
    st.markdown('<div class="info-box">המדריך מפרט את אופן מילוי קבצי האקסל בצורה המיטבית לאלגוריתם.</div>', unsafe_allow_html=True)

with tab_run:
    col_u1, col_u2 = st.columns(2)
    with col_u1: sf = st.file_uploader("📂 קובץ חיילים (xlsx)", type="xlsx")
    with col_u2: tf = st.file_uploader("📂 קובץ משימות (xlsx)", type="xlsx")

    if sf and tf:
        s_df = pd.read_excel(sf)
        t_df = pd.read_excel(tf)
        soldiers = [Soldier(r["מספר_אישי"], r["שם"], r.get("פטורים","")) for _, r in s_df.iterrows()]
        tasks = [Task(r["קוד_משימה"], r["שם"], r["כוח_אדם_נדרש"], r.get("משך_משמרת"), r.get("שעות_מנוחה_אחרי"), r.get("אישור_חפיפה"), r.get("שעות_פעילות")) for _, r in t_df.iterrows()]
        
        if st.button('⚙️ צור שבצ"ק ונתח תובנות', use_container_width=True):
            final_df, status = solve_scheduling(soldiers, tasks)
            if final_df is not None:
                st.success("✅ שיבוץ נוצר בהצלחה!")
                
                # כרטיסי המדד (החלק שתוקן למניעת חפיפה)
                avg = final_df["סך שעות"].mean()
                gap = final_df["סך שעות"].max() - final_df["סך שעות"].min()
                st.markdown(f"""
                <div class="metric-row">
                  <div class="metric-card"><div class="mc-label">חיילים</div><div class="mc-value">{len(soldiers)}</div></div>
                  <div class="metric-card"><div class="mc-label">ממוצע שעות</div><div class="mc-value">{avg:.1f}</div></div>
                  <div class="metric-card"><div class="mc-label">פער הוגנות</div><div class="mc-value">{gap}</div></div>
                </div>
                """, unsafe_allow_html=True)

                st.subheader("📅 לוח השיבוץ הסופי")
                st.table(final_df)
            else:
                st.error(status)
