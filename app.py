...
# ══════════════════════════════════════════════════════════════════
# 2. מחלקות הנתונים (Objects) - כולל תמיכה בתפקידים
# ══════════════════════════════════════════════════════════════════
class Soldier:
...
# ══════════════════════════════════════════════════════════════════
# 4. המנוע המתמטי - כולל אילוץ תפקידים ושינה
# ══════════════════════════════════════════════════════════════════
...
with tab_templates:
    ...
    c1, c2 = st.columns(2)
    with c1: st.download_button("הורד תבנית חיילים", data=to_excel_styled(s_ex, "Soldiers", False), file_name="Soldiers_with_Roles.xlsx")
    with c2: st.download_button("הורד תבנית משימות", data=to_excel_styled(t_ex, "Tasks_with_Roles.xlsx", False), file_name="Tasks_with_Roles.xlsx")
...
with tab_guide:
    ...
    st.markdown("""<table class="guide-table">
        ...
        <tr><td><b>תפקידים</b></td><td>רשימת ההכשרות של החייל (מופרד בפסיק). לדוגמה: <b>נהג, חובש</b>. המערכת תבדוק עמודה זו מול דרישות המשימה.</td></tr>
    ...
    st.markdown("""<table class="guide-table">
        <tr><th>עמודה</th><th>הסבר מפורט</th></tr>
        <tr><td><b>תפקיד_נדרש</b></td><td>התפקיד שחייב להיות לחייל כדי לבצע משימה זו (למשל <b>נהג</b> לסיור). אם ריק - כל חייל יכול לבצע.</td></tr>
        <tr><td><b>משך_משמרת</b></td><td>שעות רצופות שהחייל "נעול" במשימה ולא יוחלף.</td></tr>
    </table>""", unsafe_allow_html=True)
    ...
    if sf and tf:
        ...
        if st.button("⚙️ צור שבצ''ק ונתח תובנות"):
            with st.spinner("מחשב שיבוץ הכולל התאמת תפקידים..."):
                ...
                # בדיקה מהירה אם חסרים תפקידים קריטיים בסד"כ
                ...
                if missing:
                    st.error(f"❌ שגיאה: חסרים חיילים עם התפקידים הבאים בסד''כ: {', '.join(missing)}. המערכת לא יכולה לאייש את המשימות.")
                else:
                    ...
                    if final_df is not None:
                        ...
                        with st.expander("💡 למה הלו\"ז נראה ככה?"):
                            st.write(f"**התאמת תפקידים:** המערכת וידאה שרק חיילים עם התפקידים המתאימים שובצו למשימות הנדרשות (למשל נהגים לסיור).")
                            st.write(f"**פערי עומס:** חייל עם תפקיד נדיר (למשל נהג יחיד) עלול לעבוד יותר בגלל דרישת התפקיד.")
                    else:
