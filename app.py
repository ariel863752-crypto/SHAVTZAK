...
try:
    ...
    <div class="app-header">
      ...
      <p>הנחיות מפקד · ולידציה מוקדמת · אבחון כשל CP-SAT · אקסל תצוגת משימות</p>
    </div>
    """, unsafe_allow_html=True)
 ...
 # ── מדריך ───────────────────────────────────────────────────────
    with tab_guide:
        ...
        st.markdown("""
        ברוכים הבאים למערכת השיבוץ החכמה! המערכת נועדה לחסוך לכם שעות של תכנון ידני, למנוע טעויות, ולדאוג לחלוקת נטל הוגנת תוך שמירה על פקודות המנוחה.
        להלן הסבר קצר על איך להשתמש במערכת בצורה הטובה ביותר:
        """)

        with st.expander("📝 שלב 1: איך ממלאים את קובץ החיילים?"):
            st.markdown("""
            קובץ זה מכיל את מצבת כוח האדם שלכם. 
            * **מספר אישי / שם מלא:** חובה למלא עבור כל חייל.
            * **פטורים:** מספרי המשימות (מס"ד) שהחייל פטור מהן לחלוטין (למשל: `101, 103`). אם אין, השאירו ריק.
            * **הסמכות:** תפקידים מיוחדים שהחייל מוסמך אליהם, מופרדים בפסיק (למשל: `חובש, נהג, מפקד`).
            * **שעות חסימה:** שעות שבהן החייל לא נמצא, בחופש, או חסום מלעבוד. 
              * *דוגמאות לכתיבה נכונה:* `8-12` (מ-08:00 עד 12:00), `22-6` (משמרת לילה חסומה), או שעות בודדות מופרדות בפסיק: `8, 9, 14`.
            """)
            
        with st.expander("📋 שלב 2: איך ממלאים את קובץ המשימות?"):
            st.markdown("""
            קובץ זה מכיל את המשימות שצריך לאייש במהלך היממה.
            * **מס"ד משימה / שם המשימה / סד"כ נדרש:** שדות חובה (כמה חיילים צריכים במקביל בעמדה).
            * **משך משמרת:** כמה שעות ברצף אורכת כל משמרת (למשל `4`).
            * **שעות מנוחה מינימליות:** חובת מנוחה *אחרי* סיום המשימה (למשל `8` שעות שינה אחרי שמירה).
            * **אישור חפיפה (True/False):** האם החייל יכול לבצע את המשימה הזו *במקביל* למשימות אחרות? (למשל: כוננות טלפונית = `True`, עמדת שמירה = `False`).
            * **שעות פעילות:** מתי המשימה קורית. אפשר לכתוב `all` ל-24/7, או טווח שעות כמו `10-18`.
            * **הסמכה נדרשת:** האם צריך `נהג` או `חובש`? (אם נדרשות כמה הסמכות במקביל, כתבו מופרד בפסיק: `נהג, חובש`).
            * **עצימות (1-3):** עד כמה המשימה שוחקת (משפיע על חלוקת הנטל ההוגנת של האלגוריתם).
            """)

        with st.expander("👨‍✈️ שלב 3: הנחיות מפקד ומשימות אד-הוק (דרך האתר)"):
            st.markdown("""
            <div class="directive-box">
            <b>הנחיות מפקד מיוחדות (NLP):</b><br>
            מאפשרות לכם לתת "פקודות ברזל" לאלגוריתם לפני הריצה, מבלי לגעת באקסל.<br>
            אתם יכולים להשתמש בבנאי המובנה, או פשוט להקליד בשפה חופשית: <i>"אריאל חייב לשמור בין 14 ל-18"</i> או <i>"תומר אסור לו לעבוד במטבח"</i>.<br>
            * <b>שים לב:</b> אם תיתן הנחיה שסותרת את חוקי האקסל (למשל לשבץ חייל כשהוא חסום לחלוטין), המערכת לא תקרוס אלא תתריע על כך באבחון השגיאות!
            </div>
            <div class="adhoc-box">
            <b>משימות אד-הוק (Ad-Hoc):</b><br>
            קפצה לכם משימה פתאומית שלא נמצאת בקובץ המשימות הרגיל? אין בעיה. הוסיפו אותה דרך האתר תחת "משימות אד-הוק" לפני ההרצה, והיא תשתקלל בשיבוץ האוטומטי כאילו הייתה שם תמיד.
            </div>
            """, unsafe_allow_html=True)

        with st.expander("💡 שלב 4: פענוח התוצאות ומערכת ההמלצות"):
            st.markdown("""
            לאחר הרצת מנוע האופטימיזציה (CP-SAT), תקבלו מספר תוצרים מרכזיים:
            * **לוחות זמנים אקטיביים:** תצוגה מדויקת של מי עושה מה ובאיזו שעה (לפי חייל או לפי משימה). ניתן להוריד מיד ל-Excel.
            * **מדדי עומס והוגנות:** גרף חכם שמוודא שאף אחד לא "נטחן" בזמן שאחרים נחים, וספירה מדויקת של שעות השינה.
            * <b style="color:#c0392b;">מערכת המלצות פרואקטיבית לפתרון חוסרים:</b><br>
              במידה וחסר כוח אדם ויש "חורים" בלוח (מסומנים כ-⚠️), המערכת לא סתם תציג שגיאה. היא תנתח <b>למה</b> חסר אדם (למשל: חסר נהג? כולם בשעות מנוחה? כולם פטורים?) ותציע לכם פעולה קונקרטית כדי לפתור את הפלונטר.
            """, unsafe_allow_html=True)
            
        st.markdown("""
        <div class="info-box">
        <b>מוכנים להתחיל?</b> עברו לטאב <b>"🚀 ביצוע שיבוץ"</b>, העלו את קבצי ה-Excel שלכם, ולחצו על "צור שבצ"ק חכם"!
        </div>
        """, unsafe_allow_html=True)
